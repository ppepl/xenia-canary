...
// Responds to queries on the given XNKID
#define XNET_QOS_LISTEN_ENABLE 0x00000001
// Rejects queries on the given XNKID
#define XNET_QOS_LISTEN_DISABLE 0x00000002
// Sets the block of data to send back to queries
#define XNET_QOS_LISTEN_SET_DATA 0x00000004
// Sets max bandwidth that query responses may consume
#define XNET_QOS_LISTEN_SET_BITSPERSEC 0x00000008
// Stops listening on given XNKID and releases memory
#define XNET_QOS_LISTEN_RELEASE 0x00000010

// Connection not started; use XNetConnect or send packet
#define XNET_CONNECT_STATUS_IDLE 0x00000000
// Connecting in progress; not complete yet
...
// Qos has finished processing this entry
#define XNET_XNQOSINFO_COMPLETE 0x01
// Target host was successfully contacted
#define XNET_XNQOSINFO_TARGET_CONTACTED 0x02
// Target host has disabled its Qos listener
#define XNET_XNQOSINFO_TARGET_DISABLED 0x04
// Target host supplied Qos data
#define XNET_XNQOSINFO_DATA_RECEIVED 0x08
// Qos has unfinished estimates for this entry
...
dword_result_t NetDll_XNetStartup_entry(dword_t caller,
  ...
  if (params) {
    ...
    switch (params->cfgFlags) {
      case XNET_STARTUP_BYPASS_SECURITY:
        XELOGI("XNetStartup XNET_STARTUP_BYPASS_SECURITY");
        break;
      case XNET_STARTUP_ALLOCATE_MAX_DGRAM_SOCKETS:
        XELOGI("XNetStartup XNET_STARTUP_ALLOCATE_MAX_DGRAM_SOCKETS");
        break;
      case XNET_STARTUP_ALLOCATE_MAX_STREAM_SOCKETS:
        XELOGI("XNetStartup XNET_STARTUP_ALLOCATE_MAX_STREAM_SOCKETS");
        break;
      case XNET_STARTUP_DISABLE_PEER_ENCRYPTION:
        XELOGI("XNetStartup XNET_STARTUP_DISABLE_PEER_ENCRYPTION");
        break;
      default:
        break;
    }
...
dword_result_t NetDll_XNetUnregisterInAddr_entry(dword_t caller, dword_t addr) {
  XELOGI("NetDll_XNetUnregisterInAddr({:08X})", addr);
...
dword_result_t NetDll_XNetSetSystemLinkPort_entry(dword_t caller,
                                                  dword_t port) {
  XELOGI("XNetSetSystemLinkPort: {}", port);
...
dword_result_t NetDll_XNetQosServiceLookup_entry(dword_t caller, dword_t flags,
                                                 dword_t event_handle,
                                                 lpdword_t pqos) {
  XELOGI("XNetQosServiceLookup({:08X}, {:08X}, {:08X}, {:08X})", caller, flags,
         event_handle, pqos.guest_address());
  ...
  if (pqos) {
    ...
    qos->info[0].flags =
        XNET_XNQOSINFO_COMPLETE | XNET_XNQOSINFO_TARGET_CONTACTED | 3;
    qos->count_pending = 0;
...
dword_result_t NetDll_XNetQosListen_entry(dword_t caller, lpvoid_t sessionId,
  ...
  XELOGI("XNetQosListen({:08X}, {:016X}, {:016X}, {}, {:08X}, {:08X})", caller,
         sessionId.host_address(), data.host_address(), data_size, r7, flags);

  if (data_size <= 0) return X_ERROR_SUCCESS;

  if (flags | XNET_QOS_LISTEN_SET_DATA) {
    XELOGI("XNetQosListen XNET_QOS_LISTEN_SET_DATA");
...
dword_result_t NetDll_XNetQosLookup_entry(
  ...
  if (qos_ptr) {
    ...
    qos->info[0].up_bits_per_sec = 13125;
    qos->info[0].down_bits_per_sec = 21058;
    qos->info[0].flags = XNET_XNQOSINFO_COMPLETE |
                         XNET_XNQOSINFO_TARGET_CONTACTED |
                         XNET_XNQOSINFO_DATA_RECEIVED;
    qos->count_pending = 0;
