...
dword_result_t XamCreateEnumeratorHandle_entry(
  ...
  if (open_message == 0x58039) {
    auto e = make_object<XStaticEnumerator<XLiveAPI::XTitleServer>>(kernel_state(),
                                                          item_count);
    auto result =
    ...
    for (const auto& server : servers) {
      ...
      item->server_address.S_un.S_addr = inet_addr(server.server_address.c_str());
      item->flags = server.flags;
      memcpy(item->server_description, server.server_description,
             strlen(server.server_description));
    }
